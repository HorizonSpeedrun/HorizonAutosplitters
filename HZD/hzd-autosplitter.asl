...
// Memory location for loading found by Canine
// Autosplitter developed by ISO, testing and data collected by Made_By_Scar and JustSaft
// Version detection from the Death Stranding and Alan Wake ASL

state("HorizonZeroDawn", "v181/7517962-Steam")
{
    byte24 aobPosition : 0x0714CDC8, 0x30, 0x120; // 3 doubles
    byte invulnerable : 0x0714CDC8, 0x30, 0x1F0, 0x60;

    uint pause : 0x0714F830, 0x20;
    byte frameTimeIGT : 0x0714F830, 0x158;
...
The other player base address should be derivable by subtracting 0x2A68 from that offset

The other windowActive base address should be derivable by adding 0x2E10 to the found offset
...
startup
{
    ...
    Action<string> DebugOutput = (text) => {
        if (vars.WriteDebug)
        {
            print("[HZD Autosplitter Debug] " + text);
        }
    };
    ...
    Action<string, List<string>> DebugOutputList = (name, list) => {
        if (vars.WriteDebug)
        {
            ...
            text += " |";
            print("[HZD Autosplitter Debug] List: " + name + " (Size:" + list.Count() + "):" + text);
        }
    };
    vars.DebugOutputList = DebugOutputList;

    Action<string> InfoOutput = (text) => {
        print("[HZD Autosplitter] " + text);
    };
...
update
{
    ...
    if (!vars.tabbedOutHelper)
    {
        vars.effectiveIGTRunning = old.frameTimeIGT > 0;
        if (old.windowActiveInd <= 2 && current.windowActiveInd == 9 && old.pause == 0)
        {
            vars.tabbedOutHelper = true;
            vars.effectiveIGTRunning = false;
        }
    }
    else
    {
        vars.effectiveIGTRunning = false;
        if (old.windowActiveInd > 2 && current.windowActiveInd == 2 && current.pause == 0 ||
            old.pause > 0 && current.pause == 0)
        {
            vars.tabbedOutHelper = false;
        }
    }
